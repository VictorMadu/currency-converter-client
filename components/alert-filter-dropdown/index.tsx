...
const handleClick = (
  ...
  alertType: "pending" | "triggered" | undefined
) => {
  Promise.resolve(getCurrenciesAlerts(undefined, undefined, alertType)).then(
    (data) => {
...
const AlertFilterDropdown = (props: IAlertFilterDropdownProps) => {
  ...
  return (
    <div className="relative w-full max-w-[8rem] font-sans">
      ...
      {showDropdown ? (
        <Dropdown>
          <Dropdown.Item onClick={() => handleClick(dispatch, undefined)}>
            All
          </Dropdown.Item>
          <Dropdown.Item onClick={() => handleClick(dispatch, "pending")}>
            Pending
          </Dropdown.Item>
          <Dropdown.Item onClick={() => handleClick(dispatch, "triggered")}>
            Triggered
          </Dropdown.Item>
        </Dropdown>
      ) : (
